...
# directory info
case_dir = os.getenv("HOME")+'/codes/e3sm-ecp/cases/'
src_dir  = os.getenv("HOME")+'/codes/e3sm-ecp/branches/master/'
...
stop_opt,stop_n,resub,walltime = 'nmonths',12,0,'2:00'

ne,npg         = 30,0
compset        = 'FC5AV1C-L' #'FSP1V1' #'FC5AV1C-H02' 
crm_nx         = 64
crm_nx_rad     = 64
crm_dx         = 1000
arch           = 'CPU'           # GPU / CPU
num_nodes      = 64              # ne4=>1, ne30=>15, ne120=>??
tasks_per_node = 18              # GPU=>18 / CPU=>64
pcols          = 64              # should be slightly larger than #CRM/(total mpi tasks)
irad           = 1
 
phys = f'SP1_{crm_nx}x1_{crm_dx}m_nxrad{crm_nx_rad}_irad{irad}'
res  = f'ne{ne}' if npg==0 else  f'ne{ne}pg{npg}'

timestamp = '20191204'

case = '.'.join(['e3sm-mmf',arch,res,compset,phys,timestamp,'test3'])
...
dtime    = 20*60   # GCM physics time step (20 minutes)
ncpl     = 86400 / dtime
#---------------------------------------------------------------------------------------------------
# Create new case
#---------------------------------------------------------------------------------------------------
if newcase :
   ...
   # Symlimk to the run directory
   os.system('ln -s `./xmlquery -value RUNDIR` run')
...
if config : 
   ...
   if True:
       cam_opt = ' -phys cam5 -use_SPCAM  -rad rrtmg -nlev 72 -microphys mg2 ' \
                ...
                +' -crm_nx_rad '+str(crm_nx_rad)+' -crm_ny_rad 1 '             \
       ...
       cam_opt = ' -crm_nx ' + str(crm_nx) + ' -crm_nx_rad ' + str(crm_nx_rad)
       os.system('./xmlchange -file env_build.xml -id CAM_CONFIG_OPTS -append -val  \"'+cam_opt+'\"' )
   #----------------------------------------------------------------------------
   #----------------------------------------------------------------------------
   ...
   if ne == 120: os.system('./xmlchange ATM_PIO_NETCDF_FORMAT=\"64bit_data\" ')
...
if submit : 
   ...
   # Turn on surface flux smoothing for non-PG runs
   if npg == 0: file.write(' srf_flux_avg = 1')

   # radiation every 30 minutes
   file.write(' iradlw = ' + str(irad) +' \n')
   file.write(' iradsw = ' + str(irad) +' \n')
